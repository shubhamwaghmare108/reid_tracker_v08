"""Low-overhead JSON/CSV audit metrics for V08.2 tracker runs."""
...
CSV_COLUMNS = ('session_id frame timestamp event_type track_id identity track_state identity_state '
               ...
               'face_person_result face_count inference_time_ms best_score second_best_score association_margin recovery_margin').split()
...
class MetricsCollector:
    """Buffered, best-effort metrics collector; metrics never alter tracking."""
    def __init__(self, enabled=True, event_logging=True, output_dir='results', log_detections=False, flush_size=500):
        self.enabled,self.event_logging,self.output_dir,self.log_detections,self.flush_size=enabled,event_logging,Path(output_dir),log_detections,flush_size
        self.events=[]; self.counters=Counter(); self.rejections=Counter(); self.track_data={}; self.identity_data=defaultdict(lambda:{'track_ids':set(),'confirmed':False,'identity_switches':0,'recovery_count':0,'in_events':0,'out_events':0})
        self.session={}; self.run_dir=None; self.csv_path=None; self.started_perf=0.; self.events_write_success=False; self.metrics_write_success=False
        self._pending_recovery=set(); self._recovery_outcomes=set()
    ...
    def start_session(self, video_name, fps, configuration):
        ...
        self.session={'session_id':f'{stamp:%Y-%m-%d}_{base}_{index:03d}','video_name':base,'started_at':stamp.isoformat(),'ended_at':None,'duration_seconds':0.,'fps':fps,'total_frames':0,'tracker_version':'V08.2'}
    ...
    def record(self,event_type,frame,timestamp=0.,track=None,**values):
        if not self.enabled:return
        increment=values.pop('count',1); track_id=values.pop('track_id',getattr(track,'tracker_id','')); identity=values.pop('identity',getattr(track,'name',''))
        # Recovery is an outcome for a specific track. A stray failure emitted
        # without a pending attempt is ignored so failure counts can never exceed
        # attempts. This also repairs legacy per-cycle failure logging.
        recovery_key=(int(track_id),) if track_id!='' else None
        if event_type=='RECOVERY_ATTEMPT' and recovery_key is not None:
            self._pending_recovery.add(recovery_key); self.counters[event_type]+=1; increment=0
            if track_id!='':
                ...
                data['last_frame']=frame; data['recovery_attempts']+=1
        elif event_type in ('RECOVERY_SUCCESS','RECOVERY_FAILED','RECOVERY_AMBIGUOUS') and recovery_key is not None:
            if recovery_key not in self._pending_recovery or recovery_key in self._recovery_outcomes:
                return
            self._recovery_outcomes.add(recovery_key); self._pending_recovery.discard(recovery_key); self.counters[event_type]+=1; increment=0
            if track_id!='':
                data=self.track_data.setdefault(int(track_id),{'track_id':int(track_id),'first_frame':frame,'last_frame':frame,'observations':0,'missed_frames':0,'occlusion_count':0,'occlusion_frames':0,'recovery_attempts':0,'successful_recoveries':0,'failed_recoveries':0,'ambiguous_recoveries':0,'identity_changes':0,'gallery_update_attempts':0,'gallery_updates_accepted':0,'gallery_updates_rejected':0})
                data['last_frame']=frame
                key={'RECOVERY_SUCCESS':'successful_recoveries','RECOVERY_FAILED':'failed_recoveries','RECOVERY_AMBIGUOUS':'ambiguous_recoveries'}[event_type]; data[key]+=1
        ...
        if event_type=='IDENTITY_CONFIRMED' and identity and identity!='Unknown':
            m=self.identity_data[identity]
            if m['confirmed'] and track_id not in m['track_ids']:self.counters['TRACK_FRAGMENT']+=1
            m['confirmed']=True
        ...
        if identity and identity!='Unknown' and event_type=='IDENTITY_SWITCH':self.identity_data[identity]['identity_switches']+=1
        if event_type=='ASSOCIATION_REJECTED':
            reason=values.get('rejection_reason','NO_SAFE_MATCH'); self.rejections[reason]+=1
        ...
        row={key:'' for key in CSV_COLUMNS}; row.update({key:value for key,value in values.items() if key in CSV_COLUMNS}); row.update(session_id=self.session.get('session_id',''),frame=frame,timestamp=f'{timestamp:.3f}',event_type=event_type,track_id=track_id,identity=identity,track_state=TRACK_STATE_NAMES.get(getattr(track,'state',''),getattr(track,'state','')),identity_state=IDENTITY_STATE_NAMES.get(getattr(track,'identity_state',''),getattr(track,'identity_state',''))); self.events.append(row)
    ...
    def finalize(self,total_frames,presence,tracks,fps):
        ...
        for track in tracks:
            data=self.track_data.setdefault(track.tracker_id,{'track_id':track.tracker_id,'first_frame':0,'last_frame':total_frames,'observations':0,'missed_frames':0,'occlusion_count':0,'occlusion_frames':0,'recovery_attempts':0,'successful_recoveries':0,'failed_recoveries':0,'ambiguous_recoveries':0,'identity_changes':0,'gallery_update_attempts':0,'gallery_updates_accepted':0,'gallery_updates_rejected':0}); data.update(identity=track.name,final_track_state=TRACK_STATE_NAMES.get(track.state,track.state),final_track_state_id=track.state,final_identity_state=IDENTITY_STATE_NAMES.get(track.identity_state,track.identity_state),final_identity_state_id=track.identity_state,missed_frames=track.missed_frames,occlusion_frames=track.occlusion_frames)
        identities=[]
