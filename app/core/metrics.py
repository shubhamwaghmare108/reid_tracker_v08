...
class MetricsCollector:
    def __init__(self,enabled=True,event_logging=True,output_dir='results',log_detections=False,flush_size=500):
        ...
        self.events=[];self.counters=Counter();self.rejections=Counter();self.track_data={};self.identity_data=defaultdict(lambda:{'track_ids':set(),'confirmed':False,'identity_switches':0,'recovery_count':0,'in_events':0,'out_events':0});self.session={};self.run_dir=None;self.csv_path=None;self.started_perf=0.;self.events_write_success=False;self.metrics_write_success=False;self._pending_recovery=set()
    ...
    def start_session(self,video_name,fps,configuration):
        ...
        self.run_dir=root/f'run_{index:03d}';self.run_dir.mkdir();self.csv_path=self.run_dir/'events.csv';self.session={'session_id':f'{stamp:%Y-%m-%d}_{base}_{index:03d}','video_name':base,'started_at':stamp.isoformat(),'ended_at':None,'duration_seconds':0.,'fps':fps,'total_frames':0,'tracker_version':'V08.2'};self.configuration=configuration;self.started_perf=time.perf_counter()
    ...
    def record(self,event_type,frame,timestamp=0.,track=None,**values):
        ...
        if event_type=='RECOVERY_ATTEMPT' and track_id!='':
            key=(int(track_id),)
            if key not in self._pending_recovery:
                self._pending_recovery.add(key);self.counters[event_type]+=1;data=self._track_default(track_id,frame);data['last_frame']=frame;data['recovery_attempts']+=1
            increment=0
        elif event_type in ('RECOVERY_SUCCESS','RECOVERY_FAILED','RECOVERY_AMBIGUOUS') and track_id!='':
            key=(int(track_id),)
            if key not in self._pending_recovery:return
            self._pending_recovery.discard(key);self.counters[event_type]+=1;data=self._track_default(track_id,frame);data['last_frame']=frame;field={'RECOVERY_SUCCESS':'successful_recoveries','RECOVERY_FAILED':'failed_recoveries','RECOVERY_AMBIGUOUS':'ambiguous_recoveries'}[event_type];data[field]+=1;increment=0
        ...
        if event_type=='IDENTITY_CONFIRMED' and identity and identity!='Unknown':
            m=self.identity_data[identity];m['confirmed']=True
        ...
        if event_type=='ASSOCIATION_REJECTED':
            reason=values.get('rejection_reason','NO_SAFE_MATCH');self.rejections[reason]+=increment;category={'REID_GATE_FAIL':'association_reject_reid','RECOVERY_REID_GATE_FAIL':'association_reject_reid','IOU_GATE_FAIL':'association_reject_iou','MOTION_GATE_FAIL':'association_reject_motion','SCALE_GATE_FAIL':'association_reject_scale','COMBINED_GATE_FAIL':'association_reject_combined','LOW_DETECTION_CONFIDENCE':'association_reject_confidence','NO_SAFE_MATCH':'association_reject_no_safe_match'}.get(reason)
    ...
    def finalize(self,total_frames,presence,tracks,fps):
        if not self.enabled:return
        self._flush();elapsed=time.perf_counter()-self.started_perf;self.session.update(ended_at=datetime.now(timezone.utc).isoformat(),total_frames=total_frames,duration_seconds=total_frames/fps if fps else 0.)
        for track in tracks:
